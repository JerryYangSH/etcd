...
func main() {
	...
	var iopsLimit int
	var grpcDebug int
	flag.IntVar(&totalKeyCount, "keycount", 100, "total key count")
	flag.BoolVar(&delete, "delete", true, "delete these keys when test completes")
	flag.IntVar(&iopsLimit, "limit", 0, "iops limit. Default 0 indicates no limit.")
	flag.IntVar(&grpcDebug, "grpcdebug", 0, "enable gRPC verbose tracing for debug")
	flag.Parse()

	log.Printf("PERF : Total key count %d, deleteEnabled %t, iopsLimit %d, grpcDebug %d\n", totalKeyCount, delete, iopsLimit, grpcDebug)
	...
	perfMixed(&cli, totalKeyCount, delete, iopsLimit, startTime)
